...
async fn send_logs(exporter: DynLogExporter, logs: Vec<ProcessedLog>) -> Result<(), Error> {
    let number_of_logs = logs.len();
    let start_time = Instant::now();
    exporter.export(logs).await?;
    tracing::info!(
        "Delivered {} log records in {}ms.",
        number_of_logs,
...
fn get_severity_level(message: &str) -> LogSeverity {
    let mut severity: LogSeverity = LogSeverity::Info;
...
mod tests {
    ...
    #[test]
    fn test_convert_to_processed_log_with_json_extraction() {
        ...
        let entry = convert_to_processed_log(log.to_string(), app_name, sub_name, &mctx, &config);
    ...
    #[test]
    fn test_convert_to_processed_log_json_fallback() {
    ...
    #[test]
    fn test_convert_to_processed_log_nested_json() {
        let log = r#"{
            "metadata": {
                "application": {
                    "name": "nested-app",
    ...
    fn test_json_path_spacing_variations() {
        ...
        for template in variations {
            let entry = convert_to_processed_log(
                log.to_string(),
                template,
                "default-sub",
    ...
    #[test]
    fn test_convert_to_processed_log_regex_fallback() {
    ...
    fn test_dynamic_fallback_when_regex_fails() {
        ...
        let entry2 = convert_to_processed_log(
            log.to_string(),
        ...
        let entry3 = convert_to_processed_log(
            log.to_string(),
