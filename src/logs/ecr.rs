            ...
            if let Some(attributes) = finding.attributes {
                ...
                for attribute in attributes {
                    ...
                    if current_name == "NO_PACKAGE" && attribute.key == "package_name" {
                        current_name = attribute_value
                            .clone()
                            .unwrap_or_else(|| "NO_PACKAGE".to_string());
                    }
                    if current_version == "NO_VERSION" && attribute.key == "package_version" {
                        current_version = attribute_value
                            .clone()
                ...
                logs.push(
                    serde_json::to_string(&root)
                        .unwrap_or_else(|_| "Failed to serialize root".to_string()),
                );
            }
        }
        if let Some(finding_severity_counts) = image_scan_findings.finding_severity_counts {
            let finding_severity_counts: Vec<String> = finding_severity_counts
                .iter()
            ...
            logs.push(
                serde_json::to_string(&current_count_root)
                    .unwrap_or_else(|_| "Failed to serialize current_count_root".to_string()),
            );
        }
    }
